...
const suite remediation = [] {
    ...
    "Tag maps accept only the documented envelope"_test                 = [] {
        ...
        auto record = zd::tag_record_from_property_map(gr::property_map{{"key", "x"}, {"value", 42}, {"ignored", 99}});
        expect(record.has_value() >> fatal);
        expect(record->key == gr::pmt::Value{"x"});
        expect(record->value == gr::pmt::Value{42});
        expect(record->srcid == gr::pmt::Value{});
    };
